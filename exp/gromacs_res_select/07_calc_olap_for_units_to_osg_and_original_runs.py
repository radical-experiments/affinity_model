...
if __name__ == "__main__":
    ...
    if 'baseline' in os.listdir(start_path):
            ...
            if listing.endswith('_cus'):
                ...
                for timestamp_listing in os.listdir(datapath):
                    if timestamp_listing.startswith('timestamps'):
                        ...
                        move_to_osg_timestamp_listing = 'move_to_osg_timestamp_'+sid+'.csv'
                ...
                ttc_wkd_fn = 'ttc_wkd_'+num_cores+'_with_to_osg.csv'
        ...
        with open(start_path+'/baseline/'+'avg_ttq_wkd_with_to_osg.csv', 'w') as f:
            writer = csv.writer(f)
            for timing in avg_ttq_timings:
                writer.writerow(timing)

        with open(start_path+'/baseline/'+'avg_ttx_wkd_with_to_osg.csv', 'w') as f:
            writer = csv.writer(f)
            for timing in avg_ttx_timings:
                writer.writerow(timing)

        with open(start_path+'/baseline/'+'avg_ttc_wkd_with_to_osg.csv', 'w') as f:
            writer = csv.writer(f)
