...
def get_timings(filepath):

    session = ra.Session(filepath, 'radical.pilot')
    ...
    session_name = "_".join(filepath.split('/')[-1].split('.')[-2:])

    with open('timestamps_'+session_name+'.csv', 'w') as f:
        writer = csv.writer(f)
    ...
    if 'baseline' in os.listdir(start_path):
        ...
        for listing in baseline_dir_ls:
            if listing.endswith('_cus'):
                ...
                os.chdir(datapath)
                for prof_listing in os.listdir(datapath):
                    if prof_listing.startswith('rp.session'):
                        print "/".join([datapath, prof_listing])
                        get_timings(prof_listing)
                os.chdir(start_path)
