...
for cores in cores_list:
    avg_red_tq_comet = sum(comet_data[cores]) / len(comet_data[cores])
    avg_red_tq_bridges = sum(bridges_data[cores]) / len(bridges_data[cores])
    
    comet_plotting_data.append(avg_red_tq_comet)
    bridges_plotting_data.append(avg_red_tq_bridges)
...
ax.set_yticklabels([i for i in range(40, 101, 10)], fontsize=16)
...
ax.set_ylabel(r'Percentage Reduction in $Tq_{task}$', fontsize=18)

#plt.show()
plt.savefig('tq_red_to_osg.pdf', bbox_inches='tight', dpi=144)
plt.savefig('tq_red_to_osg.png', bbox_inches='tight', dpi=144)
