...
with open('./baseline/red_ttc_osg_to_comet.csv') as f:
    reader = csv.reader(f)
    for row in reader:
        stripped_row = [elem for elem in row if elem]
        comet_data[int(stripped_row[0])] = map(float, stripped_row[1:])

with open('./baseline/red_ttc_osg_to_bridges.csv') as f:
    reader = csv.reader(f)
    for row in reader:
        stripped_row = [elem for elem in row if elem]
...
ax.errorbar(x, bridges_plotting_data, yerr=bridges_plotting_yerr, linestyle='--', linewidth=1.5, label='Bridges', color='green', capsize=5, capthick=1)
ax.errorbar(x, comet_plotting_data, yerr=comet_plotting_yerr, linestyle='--', linewidth=1.5, label='Comet', color='red', capsize=5, capthick=1)

ax.set_xticklabels(cores_list, fontsize=16)
ax.set_xticks(x, minor=False)
ax.set_yticklabels([i for i in range(0, 101, 20)], fontsize=16)
ax.set_xlim(x[0] - 0.1, x[-1] + 0.1)
ax.set_ylim(0, 100)

ax.legend()
ax.set_title('Improvement when tasks from OSG to Comet or to Bridges', fontsize=20)
ax.set_xlabel('Number of cores', fontsize=18)
ax.set_ylabel(r'Percentage Reduction in $TTC_{task}$', fontsize=18)

#plt.show()
plt.savefig('ttc_red_from_osg.pdf', bbox_inches='tight', dpi=144)
plt.savefig('ttc_red_from_osg.png', bbox_inches='tight', dpi=144)
