...
if __name__ == "__main__":
    ...
    if 'baseline' in os.listdir(start_path):
        ...
        for listing in baseline_dir_ls:
            if listing.endswith('_cus'):
                ...
                for timestamp_listing in os.listdir(datapath):
                    if timestamp_listing.startswith('move_to_osg_timestamp'):
                ...
                ttc_wkd_fn = 'ttc_wkd_'+num_cores+'_to_osg_only.csv'
                ...
                if not ttc_timings:
                    avg_ttq_timing = [int(num_cores), -1.0, -1.0, -1.0]
                    avg_ttx_timing = [int(num_cores), -1.0, -1.0, -1.0]
                    avg_ttc_timing = [int(num_cores), -1.0, -1.0, -1.0]
        ...
        with open(start_path+'/baseline/'+'avg_ttq_wkd_to_osg_only.csv', 'w') as f:
            writer = csv.writer(f)
            for timing in avg_ttq_timings:
                writer.writerow(timing)

        with open(start_path+'/baseline/'+'avg_ttx_wkd_to_osg_only.csv', 'w') as f:
            writer = csv.writer(f)
            for timing in avg_ttx_timings:
                writer.writerow(timing)

        with open(start_path+'/baseline/'+'avg_ttc_wkd_to_osg_only.csv', 'w') as f:
            writer = csv.writer(f)
